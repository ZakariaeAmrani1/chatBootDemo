...
export class DataManager {
  ...
  static addMessage(message: Message): Message {
    ...
    const chatIndex = data.chats.findIndex(chat => chat.id === message.chatId);
    if (chatIndex !== -1) {
      data.chats[chatIndex].messageCount = data.messages.filter(m => m.chatId === message.chatId).length;
