          ...
          <SelectValue>
            {selectedModelData && (
              <div className="flex items-center gap-2">
                <div className={cn("w-2 h-2 rounded-full", selectedModelData.bgColor.replace('bg-', 'bg-').replace('dark:bg-', '').split(' ')[0])} />
          ...
          {models.map((model) => (
            <SelectItem key={model.id} value={model.id} className="p-3">
              <div className="flex items-start gap-3 w-full">
                <div className={cn("w-3 h-3 rounded-full mt-1", model.bgColor.replace('bg-', 'bg-').replace('dark:bg-', '').split(' ')[0])} />
